...
date
sudo mount -t cifs //192.168.1.XX/Data/Jeedom /home/pi/airportShare -o credentials=/home/pi/.smbcred,auto,sec=ntlm,vers=1.0 >> /home/pi/Scripts/Backup/backup-Jeedom$(date +%Y%m%d).log 2>&1
...
if [ "$retval" = '' ]
then
	echo "successful copy (jeedom-backup.sh)" | /home/pi/Scripts/Telegram/tg.sh
        echo "*** $(date) successful copy ***" >> /home/pi/Scripts/Backup/backup-Jeedom$(date +%Y%m%d).log
else
	echo "copy failed (jeedom-backup.sh)" | /home/pi/Scripts/Telegram/tg.sh
	echo $retval >> /home/pi/Scripts/Backup/backup-Jeedom$(date +%Y%m%d).log
        echo "*** $(date) copy failed ***" >> /home/pi/Scripts/Backup/backup-Jeedom$(date +%Y%m%d).log
fi
